...
library(tidyverse)
data <- readRDS("data/dataFromNumeric.rds")
```
...
## Feedback JRH: Es ist eine Mini-Kleinigkeit, aber ich würde den Datensatz nicht data nennen, sondern df, damit es immer genau so ist wie in den Videos. Es ist aber kein Fehler. 
## Korrelation hatten wir in der Vorlesung noch nicht, daher können Sie das noch nicht wissen, aber Pearson-Korrelation ist hier falsch, weil ccrb1 nur ein einzelnes Item ist. 
```
...
## Feedback JRH: Zu Nutzungshäufigkeit sozialer Medien fehlt aktuell noch eine Variable im Datacleaning, die Sie z.B. NHSM nennen könnten. Dann wäre der Code einfach:
# cor.test(data$Verhaltensaenderung, data$NHSM)
```
...
## Feedback JRH: Sehr stark! Ist aber leicht daneben. Im Datacleaning machen Sie aus ccs1 einen ordered factor. Außerdem fehlt hinter der Bedingung ein Komma, wenn Sie so filtern wollen. 
## 
gruppe_vegan <- data[data$ccs1=="vegan",]
gruppe_nicht_vegan <- data[data$ccs1!="vegan",]
...
## Feedback JRH: Genau so wie die Hypothese davor. 
```
